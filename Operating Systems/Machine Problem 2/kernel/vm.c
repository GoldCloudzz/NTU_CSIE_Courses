...
void vmprint_tree(pagetable_t pagetable, int depth, char* prefix, long long vm){
  int len = strlen(prefix);
  char new_prefix[32];
  strncpy(new_prefix, prefix, 32);
  ...
  for (i = 0; i <= last_idx; i++){
    ...
    tmp = pagetable[i];
    printf("%s+-- %d: pte=%p va=%p", prefix, i, &pagetable[i], (pte_t* ) (vm + i * diff));
    if (pagetable[i] & PTE_V)
      printf(" pa=%p", PTE2PA(tmp));
    else if (pagetable[i] & PTE_S)
      printf(" blockno=%p", PTE2BLOCKNO(tmp));
    print_bits(tmp);

    if (i == last_idx)
      strncpy((new_prefix + len), "    ", 32);
    else
      strncpy((new_prefix + len), "|   ", 32);

    if (depth < 3)
      vmprint_tree((pagetable_t) PTE2PA(tmp), (depth + 1), new_prefix, (vm + i * diff));
  }
}
