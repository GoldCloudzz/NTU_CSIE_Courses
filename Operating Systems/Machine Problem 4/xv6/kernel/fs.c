...
bmap(struct inode *ip, uint bn)
{
  ...
  if (bn < NDOUINDIRECT * 2){
    ...
    uint nflayer = bn / NINDIRECT;
    bpp = bread(ip->dev, addr);
    bpa = (uint *) bpp->data;

    if ((addr = bpa[nflayer]) == 0){
      bpa[nflayer] = addr = balloc(ip->dev);
      log_write(bpp);
    }
    brelse(bpp);

    uint nslayer = bn - nflayer * NINDIRECT;
    bp = bread(ip->dev, addr);
    ba = (uint *) bp->data;

    if ((addr = ba[nslayer]) == 0){
      ba[nslayer] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
...
itrunc(struct inode *ip)
{
  ...
  for (k = 1; k < 3; k++){
    if (ip->addrs[NDIRECT + k]){
      bpp = bread(ip->dev, ip->addrs[NDIRECT + k]);
      bpa = (uint *) bpp->data;
...
namex(char *path, int nameiparent, char *name)
{
  ...
  while((path = skipelem(path, name)) != 0){
    ...
    if (ip->type == T_SYMLINK){
      ...
      readi(ip, 0, (uint64) temp, sizeof(int), (len + 1));
      iunlockput(ip);
      ...
      strncpy(cur_path, temp, 256);
      path = cur_path;

      if (depth > 20 || strlen(path) > 128)
        return 0;
